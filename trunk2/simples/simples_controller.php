...
// Set cookie stuff
$this->cookieName = Configure::read('Simples.cookie.name');
$this->cookieTime = Configure::read('Simples.cookie.expire');
$this->cookieDomain = '.' . getenv('SERVER_NAME');
        
        // Gzip output if debug is lower then 1 and the use is allowed
        if (Configure::read('debug') < 1 && Configure::read('Simples.useGzip')) {
        $this->_gzipOutput();
        }
    }
    ...
    // Set view parameters (CmsHelper uses some of these for example)
        ...
        $this->params['Simples']['view'] = $params;
    $this->set($params);
    }
...
Configure::write('Simples.user_id', $user['User']['id']);
